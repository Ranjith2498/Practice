...
/**
 * Here we use cyclic barrier to make all threads synchronize. Means to come at the same point
 * If we call cyclicBarrier.await()  on each method. Then at that level all threads will wait for all threads
 * to reach that point.
 * Means if we call on that each thread the all thread will pause execution at that level.
	    ...
	    static class Player implements Runnable {
	        ...
	        public void run() {
	            try {
	                ...
	                System.out.println("EVrrone reached here");
